...
export const apiClient = {
  ...
  gscQueries:  (siteIds, days = 30, limit = 100) => apiFetch(
    `/gsc/queries.php?site_ids=${encodeURIComponent(siteIds.join(','))}&days=${days}&limit=${limit}`,
    { _timeout: 300_000 }
  ),
  gscPages:  (siteIds, days = 30, limit = 100) => apiFetch(
  ...
  gscCountries:  (siteIds, days = 30, limit = 20) => apiFetch(
    `/gsc/countries.php?site_ids=${encodeURIComponent(siteIds.join(','))}&days=${days}&limit=${limit}`,
    { _timeout: 300_000 }
  ),
  // Dashboard
