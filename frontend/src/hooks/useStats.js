...
export function useGscDevices(siteIds = [], days = 30, enabled = true) {
  return useQuery({
    queryKey: ["gsc-devices", siteIds, days],
    queryFn:  () => apiClient.gscDevices(siteIds, days),
    enabled:  enabled && siteIds.length > 0,
    staleTime: 10 * 60_000,
    retry: false,
  });
...
export function useGscCountries(siteIds = [], days = 30, limit = 20, enabled = true) {
  return useQuery({
    queryKey: ["gsc-countries", siteIds, days, limit],
    queryFn:  () => apiClient.gscCountries(siteIds, days, limit),
    enabled:  enabled && siteIds.length > 0,
    staleTime: 10 * 60_000,
    retry: false,
  });
