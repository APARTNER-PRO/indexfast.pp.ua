...
ALTER TABLE `users`
  ADD INDEX `idx_plan_active`    (`plan`, `is_active`),
...
ALTER TABLE `tokens`
  ADD INDEX `idx_cleanup` (`expires_at`, `used_at`),
  -- Швидка інвалідація refresh токенів конкретного юзера
  ADD INDEX `idx_user_type_expires` (`user_id`, `type`, `expires_at`);
...
ALTER TABLE `daily_usage`
  ADD INDEX `idx_user_date_sent` (`user_id`, `usage_date`, `urls_sent`),
  ADD INDEX `idx_usage_date`     (`usage_date`);  -- cleanup по даті
...
ALTER TABLE `jobs`
  ADD INDEX `idx_id_user` (`id`, `user_id`);
...
--   ORDER BY priority ASC, available_at ASC LIMIT 1 FOR UPDATE;
