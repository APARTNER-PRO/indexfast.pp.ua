...
// ── Забезпечуємо існування таблиці для лімітів Service Account
try {
    DB::exec("CREATE TABLE IF NOT EXISTS `sa_daily_usage` (
        `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
        `service_account_email` VARCHAR(255) NOT NULL,
        `usage_date` DATE NOT NULL,
        `urls_sent` INT UNSIGNED NOT NULL DEFAULT 0,
        PRIMARY KEY (`id`),
        UNIQUE KEY `uq_sa_date` (`service_account_email`, `usage_date`),
        INDEX `idx_usage_date` (`usage_date`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
} catch (Throwable $e) {
    log_msg("WARNING: Failed to ensure sa_daily_usage table exists: " . $e->getMessage());
}
...
function processJob(array $job): void {
    ...
    foreach (array_chunk($urls, GOOGLE_BATCH) as $batch) {
        ...
        // ── Перевіряємо ліміт service_account на тарифі agency (200 на день)
        if ($plan === 'agency' && !empty($saEmail)) {
            $today = date('Y-m-d');
            $saLimit = 200;
            $saSentToday = (int)DB::value(
                "SELECT urls_sent FROM sa_daily_usage WHERE service_account_email = ? AND usage_date = ?",
                [$saEmail, $today]
            );

            if ($saSentToday >= $saLimit) {
                log_msg("Job #{$jobId}: Service Account {$saEmail} limit of {$saLimit} reached today. Postponing remaining URLs to tomorrow.");
                $remainingUrls = array_slice($urls, $sent + $failed);
                postponeRemainingToNextDay($jobId, $userId, $siteId, $remainingUrls, $plan);
                break;
            }

            // Якщо ліміт наближається, обрізаємо поточний батч
            $allowedCount = $saLimit - $saSentToday;
            if ($allowedCount < count($batch)) {
                log_msg("Job #{$jobId}: Slicing batch to {$allowedCount} URLs to respect Service Account limit.");
                $batch = array_slice($batch, 0, $allowedCount);
            }
        }
        ...
        if (!empty($results)) {
            ...
            $processedCount = 0;   // Лічильник для успішно опрацьованих/надісланих (без 429)
            ...
            foreach ($results as $url => $result) {
                ...
                $processedCount++; // Цей URL дійсно пішов в Google (200 або помилка, але не 429)
            ...
            // Оновлюємо статистику Service Account
            if ($plan === 'agency' && !empty($saEmail) && $processedCount > 0) {
                $today = date('Y-m-d');
                DB::exec(
                    "INSERT INTO sa_daily_usage (service_account_email, usage_date, urls_sent)
                     VALUES (?, ?, ?)
                     ON DUPLICATE KEY UPDATE urls_sent = urls_sent + ?",
                    [$saEmail, $today, $processedCount, $processedCount]
                );
            }
...
function postponeRemainingToNextDay(int $jobId, int $userId, int $siteId, array $remainingUrls, string $plan): void {
    if (empty($remainingUrls)) return;
    // Створюємо новий job для залишкових URL на наступний день (00:00:00)
    $newJobId = DB::exec(
        "INSERT INTO jobs (user_id, site_id, type, payload, status, total, priority, available_at)
         VALUES (?, ?, 'index_urls', ?, 'pending', ?, ?, DATE_ADD(CURDATE(), INTERVAL 1 DAY))",
        [$userId, $siteId, json_encode(['urls' => $remainingUrls]),
         count($remainingUrls), Plans::jobPriority($plan)]
    );
    ...
    if ($hasUrlHash) {
        $hashPlaceholders = implode(',', array_fill(0, count($remainingUrls), 'SHA2(?,256)'));
        DB::exec(
            "UPDATE indexing_log SET job_id=?, status='pending' WHERE job_id=? AND status='pending'
             AND url_hash IN ({$hashPlaceholders})",
            array_merge([$newJobId, $jobId], $remainingUrls)
        );
    } else {
        $placeholders = implode(',', array_fill(0, count($remainingUrls), '?'));
        DB::exec(
            "UPDATE indexing_log SET job_id=?, status='pending' WHERE job_id=? AND status='pending'
             AND url IN ({$placeholders})",
            array_merge([$newJobId, $jobId], $remainingUrls)
    ...
    // Повертаємо ліміт для непроведених URL
    Plans::release($userId, count($remainingUrls));

    log_msg("Postponed " . count($remainingUrls) . " URLs to next day in new job #{$newJobId} due to Service Account daily limit.");
}
